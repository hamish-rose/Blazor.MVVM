...
namespace Blazor.MVVM.ExampleComponents.Components.Tables {
    ...
    public class VirtualPagedTableViewModel<TItem> : ComponentViewModelBase, IPagedCollectionViewModel {
        ...
        public async Task NextPageAsync() {
            ...
            (IEnumerable<TItem>? items, int totalItems) = await ItemsProvider(new PageRequest(
        ...
        protected override async Task InitCoreAsync() {
            (IEnumerable<TItem>? items, int totalItems) = await ItemsProvider.Invoke(new PageRequest(0,
                PageSize,
                string.Empty,
                string.Empty));
